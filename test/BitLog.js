...
contract('BitLot', (accounts) => {
    ...
    it('should accept new commits', async() => {
        ...
        const commit = await contractInstance.addCommit(0xAD312, user1, {from: user1});
    ...
    it('it should get the author from the commit id', async() => {
        const contractInstance = await BitLog.deployed();
        const commit = await contractInstance.addCommit(0xAD312, user2, {from: user2});
        const hash = calculateHash(0, user2);
        const commitId = await contractInstance.getCommitId(hash);
        const author = await contractInstance.getCommitAuthor(commitId);
        assert.equal(author, user2);
    });

    it('should get the correct number of commits', async() => {
        const contractInstance = await BitLog.deployed();
        const commit1 = await contractInstance.addCommit(0xAD312, user3, {from: user3});
        const commit2 = await contractInstance.addCommit(0xFF34A, user3, {from: user3});
        const commit3 = await contractInstance.addCommit(0x32B51, user4, {from: user4});
